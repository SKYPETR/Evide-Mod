function OreDetector(name, col)
{
	if(col == null)col = Color.valueOf("4b95ff")
	...
	let block = extend(Block, name, {
		...
		drawPlace(x, y, rotation, valid)
		{
        	...
        	Drawf.dashCircle(x * Vars.tilesize + this.offset, y * Vars.tilesize + this.offset, this.range, col);
    	},
    ...
    let build = () => extend(Building, {
        ...
        updateTile()
		{
			//print("1 "+ block.drillEfficiencyMultiplier > 1 + "\n2 " + this.drillEfficiencyMultiplier + "\n3 " + this.canConsume())
            if(this.canConsume() && block.drillEfficiencyMultiplier > 1)
			{
                /*Vars.indexer.eachBlock(
					this,
					this.range(),
					other => other.block instanceof Drill && other.block.canOverdrive && (typeof other.block.drill === "function"),
                    other => other.applyBoost(this.efficiency * block.drillEfficiencyMultiplier, 10)
				);*/
Vars.indexer.eachBlock(
    this,
    this.range(),
    ...
    new JavaAdapter(Boolf, {
        get: function(other) {
            return other != null &&
                ...
                typeof other.block.drill === "function";
        }
    }),
    ...
    new JavaAdapter(Cons, {
        ...
        locateOres(radius)
        {
            ...
            this.tile.circle(radius / Vars.tilesize, Cons({
				get(ore)
				{
					if(ore != null && ore.overlay() != null && ore.overlay() instanceof OverlayFloor && typeof ore.overlay().getCustomDrop === "function")
					{
            ...
            for(let i = 0; i < this.detectedOres.size; i++)
			{
                ...
                if(ore.block() != Blocks.air || ore.overlay() == Blocks.air)continue;
				...
				if(ore == hoverTile && ore.block() != null && cond)
				{
					Draw.z(Layer.max);
					Draw.alpha(1);
					Draw.rect(u.getCustomDrop().uiIcon, ore.x * 8, ore.y * 8 + 8);
				}
            }
        },
        ...
        onDestroyed()
		{
            ...
            Vars.indexer.eachBlock(
				this,
				this.range(),
                b => b instanceof Drill.DrillBuild && b.enabled() && b.potentialEfficiency > 0 && (typeof b.block === "function"),
                b => {
...
let testd = new OreDetector("testd")
testd.size = 2